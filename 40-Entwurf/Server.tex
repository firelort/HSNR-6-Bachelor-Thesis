...
Der Webserver beinhaltet sowohl die Logik als auch die Darstellung. Bei einer Anfrage an den Webserver wird eine Antwort bestimmt und diese dann in eine Vorlage beziehungsweise ein HTML Dokument eingebettet. Danach wird die Antwort zurück an den Nutzer gesendet.
...
In der Komponente Datenbank Migrationen sind Migrationsskripts hinterlegt, welche die Datenbank Iterationen festhalten. Diese Skripts sollen genutzt werden, um die verwendete Datenbanken einfach mit dem benötigten Schema zu initialisieren. 
...
\includegraphics[width=\linewidth]{entwurf/gis/dfd-gis-login}
...
\includegraphics{entwurf/gis/dfd-gis-token-refresh}
\captionof{figure}{Datenfluss während eines Logins (Datenflussdiagramm)}
...
Des Weiteren werden alle ausgestellten Tokens für die Dauer der Gültigkeit in der Redis-Datenbank gespeichert, um die Tokens bei späteren Anfragen zu validieren. 

\subsubsection{Authentifizierung}

\includegraphics[width=\linewidth]{entwurf/gis/dfd-gis-role-token}
\captionof{figure}{Datenfluss der Authentifizierung und Autorisierung (Datenflussdiagramm)}

Die Authentifizierung des Benutzers wird über die in \autoref{subsub:login} erwähnten Tokens realisiert. Die Tokens müssen vom Client bei jeder Anfrage mitgesendet werden und enthalten neben der Nutzerkennung weiter Informationen zu Berechtigungen. 
...
Mit den im Token codierten Berechtigungen kann der Server die Autorisierung der vom Nutzer angefragten Aktion durchführen.

\paragraph{Tokenvalidierung}
\mbox{}\\ % Damit die Grafik in der neuen Zeile angezeigt wird

\begin{center}
	\includegraphics{entwurf/gis/dfd-gis-token-validation}
...
Dazu erhalten diese einen Zeitstempel der Ausstellung und des Ablaufs. Sollte der Nutzer einen Logout durchführen, wird der Token in der Datenbank als Widerrufen markiert. 

\subsubsection{Registierung von GameClients}
\begin{center}
	\includegraphics{entwurf/gis/dfd-gis-registierung}
	\captionof{figure}{Datenfluss der Authentifizierung und Autorisierung (Datenflussdiagramm)}
\end{center}

Die teilnehmenden GameClients sollen aufgrund der Anforderung nicht länger hartkodiert in der Anwendung vorliegen, sondern dynamisch hinterlegt werden. Deshalb müssen die GameClients sich vor Spielstart registrieren.

\paragraph{Flaggengenerierung}
Durch die benötigte Registrierung ist es möglich, dass der Algorithmus zur Flaggenerierung verändert wird. Diese soll geschehen damit die Flags nicht länger pro Semester, sondern im besten Fall für immer einzigartig sind.
...
Die Idee, dass die Flaggenerierung nur auf dem Server stattfindet, wurde verworfen, da der Nutzer auf dem Client die Möglichkeit hat die Flags bei der Übertragung abzufangen und abzugeben. Eine Verschlüsselung oder andere Verfahren würden keinen Mehrwert in puncto Sicherheit liefern, da der Nutzer Root-Rechte auf seinem System hat und so auf eventuell vorhandene PGP Keys zugreifen kann. 

\subsubsection{Flag Abgabe}
...
Gruppen können alle eigenen Flags sowie gegnerische Flags, welche auf den GameClients verteilt sind, abgeben. Gegnerische Flagshop Flags können nicht abgegeben werden, da diese keine schützenswerten

Des Weiteren muss geprüft werden, ob die Abgabe im Angriffszeitraum geschieht. Andernfalls soll die Abgabe nicht weiter verarbeitet werden und bei einer Abgabe während der Discoverzeit eine Strafe ausgesprochen werden.
...
Des Weiteren sollen die Betreuenden Strafen an Gruppen, welche die Regeln gebrochen haben, Strafen verteilen. Hier muss die Gruppe, der Grund der Bestrafung sowie die Anzahl der Strafpunkte übermittelt werden.

\subsubsection{Nutzerverwaltung}

Administrierende und Betreuungspersonal sollen die Möglichkeit erhalten, neue Nutzer anzulegen, zu bearbeiten und zu löschen. Wichtig hierbei ist, dass das Betreuungspersonal nur Nutzer der Rolle Spieler verwalten können.

Des Weiteren ist ein Import der Spielerzugänge zu realisieren, damit die generierte Zugänge einfach übernommen werden können.
...
Die betreuenden Personen des Versuchs, sollen die Möglichkeit erhalten einzelne Scan-Operationen zu (de-)aktivieren ohne, dass eine Änderung an der Software vorgenommen werden muss. Des Weiteren soll die Gewichtung der einzelnen Services jederzeit änderbar sein.
...
Studierende können die verfassten Notizen einsehen, um Hinweise oder Anweisung zum Versuch zu erhalten.

\subsubsection{Flagshop}
(todo: Bild)

Der Flashop muss die Verwaltung von Flagshop-Nutzern und Flagshop-Paketen sowie einen Kauf der Flagshop-Pakete ermöglichen.
...
Studierende können Flagshop-Pakete erwerben. Besonderheit hierbei soll sein, dass diese einen Preis mitsenden und so die Flagshop-Pakete für einen anderen Preis als angegeben erwerben können.

\subsubsection{Challenges}
(todo: Bild)

Die Studierenden müssen Informationen zu Challenge sowie deren Ablageort abrufen können. Des Weiteren müssen sie die Challenge abschließen können. Hierfür sollen die Challenges am Ende ein Passwort mitteilen, welches ebenfalls in der Datenbank vorhanden ist. Die Studierenden können dieses Passwort dann als Beweis der Lösung der Challenge an das GIS übermitteln.

Die Challenges werden entweder mit der SPA ausgeliefert oder sollen von einem Webserver zur Verfügung gestellt werden. Sie werden nicht über das GIS verteilt, da dies als API nur Daten und keine Ressourcen an den Client übermittelt.

\subsection{Reverse Proxy}
Für das Betreiben wird auf den bereits im EZS Labor verwendeten Apache HTTP-Server zurückgegriffen. Durch die Entscheidung bleibt dieser als einziger Einstiegspunkt für Anfragen zuständig. So kann die Protokollierung der Anfragen sowie die Verschlüsselung der Verbindung an die Webserver Software abgegeben.

Der Reverse Proxy muss bei der Weiterleitung der Anfrage an das GIS die eigentliche IP-Adresse der Anfrage mitteilen(todo: replace mitteilen). Dies wird benötigt, um die zugehörige Gruppe, bei der Registrierung und dem Login zu bestimmen. Die originale IP-Adresse der Anfrage soll in einen HTTP Header gespeichert werden.
...
Bei einem Spoofing-Angriff versendet die angreifende Person Anfragen für andere IP-Adressen und erhält die Antwort. So kann diese im Namen anderer agieren und den Angegriffenen beispielsweise Minuspunkte verschaffen.